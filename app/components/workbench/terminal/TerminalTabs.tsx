...
export default function TerminalTabs({
  ...
  const [terminalStates, setTerminalStates] = useState<Record<string, {
    isConnected: boolean;
    lastActivity: Date;
    output: string[];
  }>>({});
  ...
  const handleTerminalReady = useCallback((tabId: string, terminal: any) => {
  ...
  const getTabDisplayTitle = useCallback((tab: TerminalTabData) => {
    if (tab.command) {
      return `${tab.title}: ${tab.command}`;
    }
    if (tab.workingDirectory) {
      const dirName = tab.workingDirectory.split('/').pop() || tab.workingDirectory;
      return `${tab.title} (${dirName})`;
    }
    return tab.title;
  }, []);
              ...
              return (
                <button
                  ...
                  className={classNames(
                    ...
                    {
                      'bg-bolt-elements-bg-depth-1 text-bolt-elements-textPrimary border-bolt-elements-borderColorActive': isActive,
                      'hover:bg-bolt-elements-bg-depth-3 text-bolt-elements-textSecondary hover:text-bolt-elements-textPrimary': !isActive,
