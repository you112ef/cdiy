...
const _viewTransition = { ease: cubicEasingFn };
...
const FileModifiedDropdown = memo(
  ({
    ...
    const filteredAndSortedFiles = useMemo(() => {
      const filtered = modifiedFiles.filter(([filePath]) => filePath.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      const totalChanges = modifiedFiles.reduce((acc, [, history]) => {
        ...
        return (
          ...
          changes.reduce(
            (changeAcc, change) => {
              if (change.added) {
                changeAcc.additions += change.value.split('\n').length;
              }

              if (change.removed) {
                changeAcc.deletions += change.value.split('\n').length;
              }
          ...
          changes.reduce((changeAcc, change) => {
            if (change.removed) {
              changeAcc += change.value.split('\n').length;
            }
                ...
                <Popover.Panel className="absolute right-0 z-20 mt-2 w-80 sm:w-96 origin-top-right rounded-xl bg-zinc-900 shadow-xl border border-zinc-700">
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between p-3 bg-zinc-800 rounded-lg mb-3">
                      <span className="text-xs text-zinc-300">{stats.totalFiles} files modified</span>
                      ...
                      {filteredAndSortedFiles.length > 0 ? (
                        filteredAndSortedFiles.map(([filePath, history]) => {
                          ...
                          const lastModified = new Date(
                            history.versions[history.versions.length - 1]?.timestamp || Date.now(),
                          );
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(
                              filteredAndSortedFiles.map(([filePath]) => filePath).join('\n'),
                            );
                        ...
                        <button
                          onClick={() => {
                            ...
                            const patch = filteredAndSortedFiles
  ...
  ({ chatStarted, isStreaming, metadata, updateChatMestaData, setSelectedElement }: WorkspaceProps) => {
    ...
    return (
      chatStarted && (
        ...
        >
          <div
            className={classNames('fixed transition-[left,width] duration-200 bolt-ease-cubic-bezier', {
